...
def test_day07_part1():
    ...
    assert len(tree.keys()) == 9
    assert len(can_carry(tree, "shiny gold")) == 4


def test_day07_part2():
    tree = build_tree(rules)
    assert count_children(tree, "shiny gold") - 1 == 32

    tree = build_tree(rules2)
    assert count_children(tree, "shiny gold") - 1 == 126
