...
"""
passports = [parse_passport(p) for p in passports.split("\n\n")]
...
invalid_passports = [parse_passport(p) for p in invalid_passports.split("\n\n")]
...
valid_passports = [parse_passport(p) for p in valid_passports.split("\n\n")]


def test_day04_part1():
    assert has_fields(passports[0]) == True
    assert has_fields(passports[1]) == False
    assert has_fields(passports[2]) == True
    assert has_fields(passports[3]) == False
    assert p1_valid_passports(passports) == 2
...
def test_day04_part2():
    ...
    assert p2_valid_passports(invalid_passports) == 0
    assert p2_valid_passports(valid_passports) == 4
